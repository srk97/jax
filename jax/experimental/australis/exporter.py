...
def serialize_executable_spec(fn):
  ...
  if is_pjit(fn):
    num_replicas = 1
    num_partitions = len(fn._lowering.compile_args['mesh'].devices.flat)  # pytype: disable=attribute-error
    use_spmd = 1
  else:
    num_replicas = 1
    num_partitions = 1
    use_spmd = 0
  tuple_args = fn._lowering.compile_args['tuple_args']  # pytype: disable=attribute-error
